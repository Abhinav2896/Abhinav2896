...
COLS = 85
# ROWS will be calculated dynamically to preserve aspect ratio
CELL_W = 8
CELL_H = 15
RAMP = " .':-~+=*cso#%@8$"  # Smoother graduated ramp

# global tuning
CONTRAST = 1.1
BRIGHTNESS = 1.0
GAMMA = 1.0           # 1.0 avoids blowing out midtones
SHARPEN = True        # sharpen slightly to catch edges like glasses
WHITE_FLOOR = 0.90    # keep more highlights
...
W, H = im.size
# Calculate ROWS to maintain the exact physical aspect ratio of the cropped image
# recognizing that each terminal character is CELL_W x CELL_H pixels
ROWS = int(round((COLS * CELL_W * H) / (CELL_H * W)))
...
# Recompute SVG dimensions based on dynamic ROWS
ART_W = COLS * CELL_W
ART_H = ROWS * CELL_H
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">abhinav@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'abhinav@github:~$ whoami <tspan fill="{INK}">Abhinav S Bisht</tspan></text>')
parts.append(f'<rect x="{PAD+314}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
