"""
Render "ABHINAV" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
# ---- geometry / grid ------------------------------------------------------
COLS = int(os.environ.get("WORDMARK_COLS", 120))
ROWS = 0               # derived from the art -- see fit()
# blank rows above and below the art. pads the panel so that it renders at the same
# height as the portrait panel when placed side-by-side in the README table.
...
TEXT = os.environ.get("WORDMARK_TEXT", "ABHINAV")

MASK_H = 450           # glyph raster height in mask px (drives voxel density)
TRACKING = 0.20        # extra letter-spacing, in em. counter gaps must survive the
...
TILT_DEG = float(os.environ.get("WORDMARK_TILT", 0.0))
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">abhinav@github: ~$ ./wordmark.sh --3d</text>')
...
def main():
    ...
    if a.mode == "spin":
        ...
        nf = a.frames or 20
        amp = math.radians(16)
        phase = math.asin(rest / amp)
        yaws = [amp * math.sin(phase + 2 * math.pi * i / nf) for i in range(nf)]
        dur = a.dur or 5.0
